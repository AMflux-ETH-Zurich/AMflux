...
def txpdo_mapping_init(node):
    ...
    node.tpdo[0].clear()  # clear any previous mappings
    node.tpdo[0].add_variable('Status Word', 0x6041, 0x00)
    node.tpdo[0].cob_id = 0x40000180 + node.id
    node.tpdo[0].trans_type = 255
    node.tpdo[0].start(period=1)
    ...
    node.tpdo[1].clear()
    node.tpdo[1].add_variable('Position Actual Value', 0x6064, 0x00)
    node.tpdo[1].add_variable('Velocity Actual Value', 0x606C, 0x00)
