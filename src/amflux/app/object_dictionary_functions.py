...
def motor_init(node, motor_type: int=None, nominal_current: int=None, current_lim: int=None, 
    ...
    node.sdo[(0x6402, 0x00)].raw = motor_type if motor_type is not None else 0x000A
    #nominal current flowing through motor windings in mA
    node.sdo[(0x3001, 0x01)].raw = nominal_current if nominal_current is not None else 15000
    #output current limit in mA
    node.sdo[(0x3001, 0x02)].raw = current_lim if current_lim is not None else 30000
    #number of pole pairs
    node.sdo[(0x3001, 0x03)].raw = pole_pairs if pole_pairs is not None else 1
    #thermal time constant of windings
    node.sdo[(0x3001, 0x04)].raw = therm_const if therm_const is not None else 40
    #torque constant of motor
    node.sdo[(0x3001, 0x05)].raw = tor_const if tor_const is not None else 0
...
def ssi_abs_encoder_init(node, data_rate: int=None, data_bits: int=None, encoding_type: int=None,
    ...
    check_init(locals())
    #SSI data rate
    node.sdo[(0x3012, 0x01)].raw = data_rate if data_rate is not None else 2000 #REFER TO RENISHAW ENCODER SHEET
    #SSI number of data bits
    node.sdo[(0x3012, 0x02)].raw = data_bits if data_bits is not None else 0x00000C00 #REFER TO RENISHAW ENCODER SHEET
    #SSI encoding type
    node.sdo[(0x3012, 0x03)].raw = encoding_type if encoding_type is not None else 0x001 #REFER TO RENISHAW ENCODER SHEET
    #SSI timeout time
    node.sdo[(0x3012, 0x05)].raw = timeout_time if timeout_time is not None else 30 #REFER TO RENISHAW ENCODER SHEET
    ...
    #SSI Power up time
    node.sdo[(0x3012, 0x08)].raw = power_up_time if power_up_time is not None else 200 #REFER TO RENISHAW ENCODER SHEET
    #SSI postition raw value, lower 32 bits
    #READ ONLY node.sdo[0x3012][0x9].raw = position_raw_value
    #SSI commutaiton offset value
    node.sdo[(0x3012, 0x0A)].raw = commutation_offset_value if commutation_offset_value is not None else 0
    #SSI position bits
    node.sdo[(0x3012, 0x0B)].raw = position_bits if position_bits is not None else  0x0000000C #REFER TO RENISHAW ENCODER SHEET, MUST BE REDUCED IF >32
    ...
    node.sdo[(0x3012, 0x0C)].raw = communication_additional_delay if communication_additional_delay is not None else -1 #REFER TO RENISHAW EN
...
def electrical_system_init(node, electrical_resistance: int=None, electrical_inductance: int=None):
    ...
    node.sdo[(0x3002, 0x01)].raw = electrical_resistance if electrical_resistance is not None else 0
    #electrical inductance of motor windings in uH
    node.sdo[(0x3002, 0x02)].raw = electrical_inductance if electrical_inductance is not None else 0
...
def current_control_parameter_init(node, p_gain: int = None, i_gain: int = None):
    ...
    node.sdo[(0x30A0, 0x01)].raw = p_gain if p_gain is not None else 1171880
    #Current controller I-gain
    node.sdo[(0x30A0, 0x02)].raw = i_gain if i_gain is not None else 3906250
...
def position_control_parameter_init(node, p_gain:int = None, i_gain: int = None, d_gain:int = None,
    ...
    node.sdo[(0x30A1, 0x01)].raw = p_gain if p_gain is not None else 1500000
    #Position controller I-gain
    node.sdo[(0x30A1, 0x02)].raw = i_gain if i_gain is not None else 780000
    #Position controller D-gain
    node.sdo[(0x30A1, 0x03)].raw = d_gain if d_gain is not None else 16000
    #Position controller Feed Forward velocity gain
    node.sdo[(0x30A1, 0x04)].raw = ffv_gain if ffv_gain is not None else 0
    #Position controller Feed Forward acceleration gain
    node.sdo[(0x30A1, 0x05)].raw = ffa_gain if ffa_gain is not None else 0
    #Position controller I-gain SI units
    node.sdo[(0x30A1, 0x09)].raw = i_gain_si_unit if i_gain_si_unit is not None else 0xFA040300
...
def velocity_control_parameter_init(node, p_gain:int = None, i_gain: int = None, 
    ...
    node.sdo[(0x30A2, 0x01)].raw = p_gain if p_gain is not None else 20000
    #Velocity controller I-gain
    node.sdo[(0x30A2, 0x02)].raw = i_gain if i_gain is not None else 500000
    #Velocity controller Feed Forward velocity gain
    node.sdo[(0x30A2, 0x03)].raw = ffv_gain if ffv_gain is not None else 0
    #Velocity controller Feed Forward acceleration gain
    node.sdo[(0x30A2, 0x04)].raw = ffa_gain if ffa_gain is not None else 0
    #Velocity controller filter cut-off frequency in HZ
    node.sdo[(0x30A2, 0x05)].raw = f_cutoff if f_cutoff is not None else 600
...
def velocity_observer_parameter_init(node, pos_corr_gain: int = None, vel_corr_gain: int = None,
    ...
    node.sdo[(0x30A3, 0x01)].raw = pos_corr_gain if pos_corr_gain is not None else 400
    #Velocity observer velocity correction gain given in mHz
    node.sdo[(0x30A3, 0x02)].raw = vel_corr_gain if vel_corr_gain is not None else 100000
    #Velocity observer load correction gain
    node.sdo[(0x30A3, 0x03)].raw = load_corr_gain if load_corr_gain is not None else 33
    #Velocity observer friction
    node.sdo[(0x30A3, 0x04)].raw = friction if friction is not None else 10
    #Velocity observer inertia
    node.sdo[(0x30A3, 0x05)].raw = inertia if inertia is not None else 1000
...
def dual_loop_position_control_parameter_init(node, main_loop_p_gain_low:int = None, main_loop_p_gain_high:int = None, 
    ...
    node.sdo[(0x30AE, 0x01)].raw = main_loop_p_gain_low if main_loop_p_gain_low is not None else 10000
    #Represents the main loop high bandwidth proportional factor
    node.sdo[(0x30AE, 0x02)].raw = main_loop_p_gain_high if main_loop_p_gain_high is not None else 100000
    #Represents the main loop gain scheduler
    node.sdo[(0x30AE, 0x03)].raw = main_loop_gain_scheduler if main_loop_gain_scheduler is not None else 12500
...
def software_position_limit(node, min_pos_limit: int = None, max_pos_limit: int = None):
    ...
    #min position limit
    node.sdo[(0x607D, 0x01)].raw = min_pos_limit if min_pos_limit is not None else 0
    #max position limit
    node.sdo[(0x607D, 0x02)].raw = max_pos_limit if max_pos_limit is not None else 0
...
def homing_speeds(node, speed_sw_srch: int = None, speed_zero_srch: int = None):
    ...
    node.sdo[(0x6099, 0x01)].raw = speed_sw_srch if speed_sw_srch is not None else 100
    #Speed for zero search given in rpm, used to search the index in a homing sequence
    node.sdo[(0x6099, 0x02)].raw = speed_zero_srch if speed_zero_srch is not None else 10
...
def interpolation_time_period(node, time_period_val: int = None, time_index: int = None):
    ...
    node.sdo[(0x60C2, 0x01)].raw = time_period_val if time_period_val is not None else 0
    #time_index
    node.sdo[(0x60C2, 0x02)].raw = time_index if time_index is not None else -3
...
def digital_outputs(node, phys_outputs: int = None):
    ...
    node.sdo[(0x60FE, 0x01)].raw = phys_outputs if phys_outputs is not None else 0
...
def digital_input_properties_init(node, polarity: int = None):
    ...
    node.sdo[(0x3141, 0x02)].raw = polarity if polarity is not None else 0x0000
...
def config_digital_inputs_init(node, input_1_config: int = None, input_2_config: int = None,
    ...
    #input 1 (DgIn1)
    node.sdo[(0x3142, 0x01)].raw = input_1_config if input_1_config is not None else 0
    #input 2 (DgIn2)
    node.sdo[(0x3142, 0x02)].raw = input_2_config if input_2_config is not None else 1
    #input 3 (DgIn3)
    node.sdo[(0x3142, 0x03)].raw = input_3_config if input_3_config is not None else 2
    #input 4 (DgIn4)
    node.sdo[(0x3142, 0x04)].raw = input_4_config if input_4_config is not None else 19
    #input 5 (HsDgIn1)
    node.sdo[(0x3142, 0x05)].raw = input_5_config if input_5_config is not None else 255
    #input 6 (HsDgIn2)
    node.sdo[(0x3142, 0x05)].raw = input_6_config if input_6_config is not None else 255
    #input 7 (HsDgIn3)
    node.sdo[(0x3142, 0x05)].raw = input_7_config if input_7_config is not None else 255
    #input 8 (HsDgIn4)
    node.sdo[(0x3142, 0x05)].raw = input_8_config if input_8_config is not None else 255
